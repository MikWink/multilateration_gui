...
class Tdoah:
    ...
    def solve(self):
        P0 = (self.bs[0][0], self.bs[1][0], self.bs[2][0])
        P1 = (self.bs[0][1], self.bs[1][1], self.bs[2][1])
        P2 = (self.bs[0][2], self.bs[1][2], self.bs[2][2])

        locations = [P0, P1, P2]
        locations_cartesian = {}

        for i, loc in enumerate(locations):
            loc = self.w2k(loc[0], loc[1], loc[2])
            locations_cartesian[f'P{i}'] = loc

        target = self.ms[0], self.ms[1], self.ms[2]

        target_cartesian = self.w2k(target[0], target[1], target[2])
    ...
    def k2w(self, X, Y, Z):
        ...
        p = np.sqrt(X ** 2 + Y ** 2)
        fi_rad = np.arctan2(Z, p * (1 - self.B))  # Initial approximation

        for _ in range(5):  # Iterate for better accuracy
            N = self.A / np.sqrt(1 - self.B * np.sin(fi_rad) ** 2)
            h = p / np.cos(fi_rad) - N
            fi_rad = np.arctan2(Z, p * (1 - self.B * (N / (N + h))))
